       ...
       app.Services.CreateScope())
{
    ...
    // 100 different authors and 10 quotes per author.
    if (db.Quotes.Count() < 1000)
    {
        var authors = Enumerable
            .Range(1, 100)
            .Select(i => $"Performance Author {i}")
            .ToList();
        ...
        foreach (var author in authors)
        {
            for (var i = 1; i <= 10; i++)
            {
                var quote = Quote.Create(
                    author,
                    $"Performance test quote {i} by {author}");

                if (quote.IsSuccess &&
                    quote.Quote is not null)
                {
                    db.Quotes.Add(quote.Quote);
                }
            }
        }
