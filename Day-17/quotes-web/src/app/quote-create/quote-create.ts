...
export class QuoteCreate {
  ...
  // Still emitted for any caller that wants it; when routed at /quotes/new
  // (the only current usage) nothing listens, since navigation below already
  // takes the user to the new quote's detail view.
      ...
      submission: {
        action: async (field) => {
          ...
          try {
            ...
            // with a friendly message — nothing raw reaches the UI here.
            return { kind: 'server', message: (err as AppError).message };
          }
