...
public static class QuoteEndpointExtensions
{
        ...
        this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            // Day 19: publish the same creation as a topic event so
            // independent subscribers (Subscription A / Subscription B)
            // each get their own copy, separate from the Day 18 local
            // queue above. A no-op publisher is registered when Service
            // Bus is not configured (see Program.cs), so this is always
            // safe to call.
            await eventPublisher.PublishQuoteCreatedAsync(
                new QuoteCreatedEvent(
                    QuoteCreatedEvent.BuildMessageId(createdQuote.Id),
                    createdQuote.Id,
                    createdQuote.Author,
                    createdQuote.Text,
