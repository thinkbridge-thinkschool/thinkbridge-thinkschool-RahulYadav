...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    // final, retry-exhausted error into a typed AppError; apiUrlInterceptor
    // rewrites '/api' to the prod origin after auth attaches its header (see
    // api-url.interceptor.ts); authInterceptor is outermost since it only
    // touches the outgoing request.
    provideHttpClient(
      withInterceptors([authInterceptor, apiUrlInterceptor, errorInterceptor, retryInterceptor]),
    ),
  ]
};
