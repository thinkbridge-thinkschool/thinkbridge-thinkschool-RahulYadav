...
public static class QuoteEndpointExtensions
{
        ...
        this IEndpointRouteBuilder app)
    {
        ...
        // Day 21: hot read fronted by HybridCache (L1 memory + L2 Redis).
        // QuoteCacheReader.GetByIdAsync is where the DB read actually
        // happens — only on a real cache miss, and HybridCache coalesces
        // concurrent misses for the same id so at most one DB read occurs
        // per cold key (see Caching/QuoteCacheReader.cs).
        group.MapGet("/{id:int}", async (
            int id,
            QuoteCacheReader cacheReader,
            CancellationToken cancellationToken) =>
        {
            var quote = await cacheReader.GetByIdAsync(
                id,
                cancellationToken);
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (deleted)
            {
                // Day 21: without this, a cached copy from before the
                // delete would keep serving GET /api/quotes/{id} for up to
                // the cache's TTL after the row was soft-deleted.
