...
public static class DiagnosticsEndpointExtensions
{
        ...
        this IEndpointRouteBuilder app)
    {
        ...
        // Day 22: counters proving the QuoteDependency resilience pipelines
        // (see QuoteDependencyResilienceExtensions) actually ran -- retry
        // attempts, circuit-breaker transitions, timeouts and bulkhead
        // rejections. No secrets or connection details are exposed here.
        group.MapGet("/resilience", (QuoteDependencyResilienceMetrics metrics) =>
        {
            return Results.Ok(new
            {
                circuitState = metrics.CircuitState,
                retryAttempts = metrics.RetryAttempts,
                circuitOpenedCount = metrics.CircuitOpenedCount,
                circuitHalfOpenedCount = metrics.CircuitHalfOpenedCount,
                circuitClosedCount = metrics.CircuitClosedCount,
                timeoutCount = metrics.TimeoutCount,
                bulkheadRejectedCount = metrics.BulkheadRejectedCount
            });
        });
