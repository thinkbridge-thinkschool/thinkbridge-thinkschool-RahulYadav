...
using QuotesApi.Modules.Collections.Presentation.Endpoints;
using QuotesApi.Modules.Notifications;
using QuotesApi.Modules.Notifications.Presentation.Endpoints;
...
// ============================================================
// Day 21: HybridCache (L1 memory + L2 Redis) for the hot quote read
// ============================================================
//
// GET /api/quotes/{id} (see QuoteEndpointExtensions) is fronted by
// QuoteCacheReader, which wraps HybridCache.GetOrCreateAsync. A cache miss
// runs the factory — the ONLY place the DB is read — and HybridCache's
// built-in stampede protection coalesces concurrent misses for the same id
// into a single factory execution, no homemade locking involved.
//
// ConnectionStrings:Redis is read from configuration/environment only and
// is never hardcoded. Left empty (as in Testing, and on a developer
// machine without Redis), AddQuoteCaching still registers HybridCache —
// it just runs L1-only, since no IDistributedCache was registered for it
// to use as L2. See Extensions/CachingExtensions.cs.
// ============================================================

builder.Services.AddQuoteCaching(
    builder.Configuration);

// ============================================================
// Day 22 Piece 2: modular monolith — bounded-context registration
// ============================================================
//
// Each module owns and registers its own repositories/handlers via a single
// AddXModule() extension (see Modules/*/*.cs) instead of Program.cs reaching
// into every module's internals. Quotes' underlying persistence predates
// this restructuring (see QuotesModule.cs); Collections is the new
// aggregate-based module; Notifications only reacts to the integration
// events the other two publish through the shared in-process event bus.
// ============================================================
...
// ============================================================
// Day 22: Polly resilience around the outbound QuoteDependency call
// ============================================================
//
// IQuoteDependencyClient (see Resilience/QuoteDependencyClient.cs) wraps an
// outbound HTTP dependency with retry + circuit breaker + timeout +
// concurrency limiter (bulkhead), via two DI-registered Polly pipelines --
// one for the idempotent GET (retries allowed) and one for the
// non-idempotent POST/create (never retried automatically). See
// QuoteDependencyResilienceExtensions for the full pipeline and the
// "Resilience"/"QuoteDependency" configuration sections in appsettings.json.
// ============================================================

builder.Services.AddQuoteDependencyResilience(
    builder.Configuration);
...
app.MapCollectionsModuleEndpoints();
